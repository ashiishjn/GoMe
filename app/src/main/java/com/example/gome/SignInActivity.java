    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        signup.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(allValid) {
                    auth.createUserWithEmailAndPassword(ml,p).addOnCompleteListener(SignInActivity.this, new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(task.isSuccessful()) {
                                ...
                                reference.child(key).child("Questions Status").setValue("0");
                                reference.child(key).child("Gratitude").setValue("");
                                reference.child(key).child("Daily Checks").setValue("");
